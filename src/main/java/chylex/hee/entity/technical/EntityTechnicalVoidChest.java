...
public class EntityTechnicalVoidChest extends EntityTechnicalBase {
    ...
    public EntityTechnicalVoidChest(World world, double x, double y, double z, EntityPlayerMP player,
            Collection<? extends EntityItem> items, boolean simulateFall) {
        super(world);
        setPosition(x, y, z);
        this.player = player;
    ...
    public void onUpdate() {
        ...
        if (items == null || items.isEmpty()
                || player == null
                || player.isDead
                || !player.playerNetServerHandler.func_147362_b().isChannelOpen())
            setDead();
        else {
            if (age == 1 && simulateFall) {
                ...
                for (Iterator<EntityItem> iter = items.iterator(); iter.hasNext();) {
                    ...
                    EntityItem copy = new EntityItem(
                            ...
                            item.posY,
                            item.posZ,
                            new ItemStack(Blocks.bedrock, 64));
