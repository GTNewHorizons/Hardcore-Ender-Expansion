...
public class EntityMobHomelandEnderman extends EntityMob implements IEndermanRenderer, IIgnoreEnderGoo {
    ...
    static {
        ...
        if (blocks.isEmpty()) {
            endermanBlockList = new Block[] { Blocks.grass, Blocks.dirt, Blocks.sand, Blocks.gravel,
                    Blocks.yellow_flower, Blocks.red_flower, Blocks.brown_mushroom, Blocks.red_mushroom, Blocks.tnt,
    ...
    public void onLivingUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            int chance = 1 + MathUtil
                    .floor(HardcoreEnderExpansion.proxy.getClientSidePlayer().getDistanceToEntity(this) / 20F);

            if (rand.nextInt(chance) == 0) {
                float colFactor = rand.nextFloat() * 0.6F + 0.4F;
                HardcoreEnderExpansion.fx.portalColor(
                        worldObj,
            ...
            if (currentTask == EndermanTask.WAIT || currentTask == EndermanTask.GET_TNT) {
                ...
                if (--currentTaskTimer == 0) {
                    ...
                    for (int attempt = 0; attempt < 100; attempt++) {
                        tmpPos.set(posX + rand.nextInt(41) - 20, -1, rand.nextInt(41) - 20)
                                .setY(worldObj.getTopSolidOrLiquidBlock(tmpPos.x, tmpPos.z) - 1);

                        if (tmpPos.getBlock(worldObj) == BlockList.end_terrain) {
                            teleportTo(
                                    tmpPos.x + 0.3D + rand.nextDouble() * 0.4D,
                                    tmpPos.y + 1D,
                                    tmpPos.z + 0.3D + rand.nextDouble() * 0.4D,
                                    true);
                            PacketPipeline.sendToAllAround(
                                    this,
                                    256D,
                                    new C20Effect(FXType.Basic.HOMELAND_ENDERMAN_TP_OVERWORLD, this));
            ...
            if (overtakeGroup == -1) { // calm situation
                if (currentTask != EndermanTask.NONE && currentTask != EndermanTask.WAIT) { // has a task
                    ...
                    if (currentTask == EndermanTask.LISTEN_TO_RECRUITER
                        ...
                        if (currentTaskTimer == 0 && currentTask == EndermanTask.RECRUIT_TO_GROUP) {
                            ...
                            if (rand.nextInt(100) < chance) {
                                ...
                                PacketPipeline.sendToAllAround(
                                        this,
                                        64D,
                                        new C21EffectEntity(FXType.Entity.HOMELAND_ENDERMAN_RECRUIT, this));
                                PacketPipeline.sendToAllAround(
                                        this,
                                        64D,
                                        new C21EffectEntity(FXType.Entity.HOMELAND_ENDERMAN_RECRUIT, target));
                                ...
                                List<EntityMobHomelandEnderman> guards = HomelandEndermen
                                        .getByHomelandRole(this, HomelandRole.GUARD);

                                for (int a = 0, amt = Math.max(3, (int) Math.round(guards.size() * 0.3D)); a
                                        < amt; a++) {
                                    ...
                                    PacketPipeline.sendToAllAround(
                                            this,
                                            256D,
                                            new C22EffectLine(FXType.Line.HOMELAND_ENDERMAN_GUARD_CALL, target, guard));

                                    if (!escaped) {
                                        guard.teleportToEntity(this);
                                        guard.setTarget(this);
                                    } else if (rand.nextInt(4) != 0) guard.teleportTo(
                                            oldX + 4D * (rand.nextDouble() - 0.5D),
                                            oldY + 2D + rand.nextDouble() * 4D,
                                            oldZ + 4D * (rand.nextDouble() - 0.5D));
                                }
                            }
                        ...
                        if (currentTaskTimer > 0 && currentTaskTimer < 20 && rand.nextInt(3) == 0) {
                            ...
                            if ((obj.distanceTo(Vec3.createVectorHelper(posX, posY, posZ)) <= 0.5D
                                    || rand.nextInt(10) == 0) && randomTpTimer > 30) {
                    ...
                    if (groupId != -1 && rand.nextInt(400) == 0
                            && (recruitCooldown < -120 || --recruitCooldown < -120)) {
                        ...
                        if (totalAmt > 8 && (rand.nextInt(5) <= 2
                                || groupAmt < (totalAmt >> 2) + rand.nextInt(totalAmt >> 3) - rand.nextInt(4))) {
                            for (int attempt = 0; attempt < 5; attempt++) {
                                EntityMobHomelandEnderman enderman = total.get(rand.nextInt(totalAmt));

                                if (enderman == this || enderman.groupId != -1
                                        || !enderman.onGround
                                        || enderman.homelandRole == HomelandRole.ISLAND_LEADERS
                                        || enderman.getDistanceToEntity(this) > 80D)
                                    continue;
                                if (groupAmt > 3 && ((enderman.homelandRole == HomelandRole.GUARD
                                        || enderman.homelandRole == HomelandRole.WORKER) && rand.nextInt(5) != 0))
                                    continue;
                                ...
                                for (int tpAttempt = 0; tpAttempt < 30; tpAttempt++) {
                                    if (teleportTo(
                                            enderman.posX + (rand.nextDouble() - 0.5D) * 2D,
                                            enderman.posY,
                                            enderman.posZ + (rand.nextDouble() - 0.5D) * 2D)) {
                                        tp = true;
                                        break;
                                    }
                                }
                        ...
                        switch (homelandRole) {
                            case ISLAND_LEADERS:
                                if (rand.nextInt(13) == 0) {
                                    ...
                                    for (int attempt = 0, pathX, pathY, pathZ; attempt < 12; attempt++) {
                                        if (attempt > 8 || rand.nextInt(6) == 0) look = Vec3.createVectorHelper(
                                                rand.nextDouble() - 0.5D,
                                                0D,
                                                rand.nextDouble() - 0.5D).normalize();
                                        ...
                                        if (worldObj.getBlock(pathX, pathY, pathZ) == BlockList.end_terrain
                                                && MathUtil.distance(posX - pathX, posY - pathY, posZ - pathZ) > 5D) {
                                            setPathToEntity(
                                                    worldObj.getEntityPathToXYZ(
                                                            this,
                                                            pathX,
                                                            pathY + 1,
                                                            pathZ,
                            ...
                            case BUSINESSMAN:
                                if (rand.nextInt(18) == 0) {
                                    List<EntityMobHomelandEnderman> businessmen = HomelandEndermen
                                            .getByHomelandRole(this, HomelandRole.BUSINESSMAN);

                                    if (!businessmen.isEmpty()) {
                                        EntityMobHomelandEnderman enderman = businessmen
                                                .get(rand.nextInt(businessmen.size()));
                                        ...
                                        if (enderman.currentTask == EndermanTask.NONE) {
                                            for (int tpAttempt = 0; tpAttempt < 20; tpAttempt++) {
                                                if (teleportTo(
                                                    ...
                                                    currentTaskTimer = enderman.currentTaskTimer = 30
                                                            + rand.nextInt(50 + rand.nextInt(80));
                                                    break;
                                                }
                                            }
                                        }
                            ...
                            case WORKER:
                                if (rand.nextInt(270) == 0
                                        && BlockPosM.tmp(this).moveUp().getBlock(worldObj) != BlockList.ender_goo) {
                                    BlockPosM tmpPos = BlockPosM.tmp();

                                    for (int attempt = 0; attempt < 50; attempt++) {
                                        tmpPos.set(posX + rand.nextInt(71) - 35, -1, posZ + rand.nextInt(71) - 35)
                                                .setY(worldObj.getTopSolidOrLiquidBlock(tmpPos.x, tmpPos.z) - 1);
                            ...
                            case OVERWORLD_EXPLORER:
                                if (rand.nextInt(140) == 0) {
                                    currentTask = EndermanTask.WAIT;
                                    currentTaskTimer = 500 + rand.nextInt(800)
                                            + rand.nextInt(1000) * (1 + rand.nextInt(4));

                                    List<EntityMobHomelandEnderman> explorers = HomelandEndermen
                                            .getByHomelandRole(this, HomelandRole.OVERWORLD_EXPLORER);

                                    for (int a = 0; a < 1 + rand.nextInt(7) && !explorers.isEmpty(); a++) {
                                        if (rand.nextInt(3) == 0) break;

                                        EntityMobHomelandEnderman enderman = explorers
                                                .remove(rand.nextInt(explorers.size()));
                                        if (enderman == this) continue;

                                        PacketPipeline.sendToAllAround(
                                                this,
                                                256D,
                                                new C20Effect(FXType.Basic.HOMELAND_ENDERMAN_TP_OVERWORLD, enderman));
            ...
            if (overtakeGroup != -1) {
                if (overtakeGroup == groupId) { // overtaking group
                            ...
                            case CHAOSMAKER:
                                if (isCarrying() && getCarrying().getItem() == Item.getItemFromBlock(Blocks.tnt)) {
                                    if (rand.nextInt(50) == 0) {
                                        ...
                                        for (int attempt = 0; attempt < 30; attempt++) {
                                            tmpPos.set(this).move(
                                                    rand.nextInt(7) - 3,
                                                    rand.nextInt(3) - 1,
                                                    rand.nextInt(7) - 3);

                                            if (tmpPos.getBlock(worldObj) == BlockList.end_terrain
                                                    && testPos.set(tmpPos).moveUp().isAir(worldObj)) {
                            ...
                            case FIGHTER:
                                if (!(entityToAttack instanceof EntityMobHomelandEnderman)) {
                                    ...
                                    if (overtakeGroupRole == OvertakeGroupRole.LEADER || rand.nextInt(4) == 0) {
                                        List<EntityMobHomelandEnderman> list = HomelandEndermen
                                                .getByHomelandRole(this, HomelandRole.ISLAND_LEADERS);
                    ...
                    if (!worldObj.getEntitiesWithinAABB(EntityTNTPrimed.class, boundingBox.expand(5D, 3D, 5D))
                            .isEmpty())
                        teleportRandomly();
                    ...
                    if (rand.nextInt(3) == 0) setGroupMember(overtakeGroup, OvertakeGroupRole.getRandomMember(rand));
                        ...
                        && ((homelandRole == HomelandRole.GUARD && rand.nextInt(30) == 0) || rand.nextInt(200) == 0)) {
            ...
            if (entityToAttack != null) {
                ...
                if (((attackTpTimer > 50 + rand.nextInt(30) && rand.nextInt(20) == 0) || attackTpTimer > 120)
                        && entityToAttack.getDistanceSqToEntity(this) < 16D) {
                    teleportRandomly(8D);
                    attackTpTimer = -80;
                } else if (entityToAttack.getDistanceSqToEntity(this) > 256D && attackTpTimer > 30
                ...
                if (currentTask == EndermanTask.NONE && entityToAttack == null
                        && ++randomTpTimer > 70 + rand.nextInt(50)) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (worldObj.difficultySetting == EnumDifficulty.PEACEFUL && source.getEntity() instanceof EntityPlayer) {
            if (super.attackEntityFrom(DamageSource.generic, amount)) {
                ...
                if (homelandRole == HomelandRole.ISLAND_LEADERS || homelandRole == HomelandRole.GUARD
                        || rand.nextInt(3) == 0) {
                    for (int attempt = 0; attempt < 10; attempt++) {
                        if (teleportRandomly()) break;
                    }
        ...
        if (source.getEntity() instanceof EntityPlayer || source.getEntity() instanceof EntityMobHomelandEnderman) {
            ...
            if (callGuards) {
                ...
                List<EntityMobHomelandEnderman> list = worldObj.getEntitiesWithinAABB(
                        EntityMobHomelandEnderman.class,
                        boundingBox.expand(guardDist, 128D, guardDist));
                ...
                for (int a = 0, amt = Math.max(2, Math.round(list.size() * guardPerc)); a < amt
                        && !list.isEmpty(); a++) {
                    ...
                    PacketPipeline.sendToAllAround(
                            this,
                            256D,
                            new C22EffectLine(FXType.Line.HOMELAND_ENDERMAN_GUARD_CALL, this, guard));
                }
            }
    ...
    public void readEntityFromNBT(NBTTagCompound nbt) {
        ...
        homelandRole = homelandRoleId >= 0 && homelandRoleId < HomelandRole.values.length
                ? HomelandRole.values[homelandRoleId]
                : null;

        if ((groupId = nbt.getLong("groupId")) != -1) {
            byte groupRoleId = nbt.getByte("groupRole");
            overtakeGroupRole = groupRoleId >= 0 && groupRoleId < OvertakeGroupRole.values.length
                    ? OvertakeGroupRole.values[groupRoleId]
                    : null;
        }
    ...
    public String getCommandSenderName() {
        return hasCustomNameTag() ? getCustomNameTag()
                : StatCollector.translateToLocal(Baconizer.mobName("entity.homelandEnderman.name"));
    }
    ...
    private boolean canTeleport() {
        if (HomelandEndermen.isOvertakeHappening(this))
            return HomelandEndermen.getOvertakeGroup(this) == groupId || rand.nextInt(100) > 30 + rand.nextInt(50)
                    + 12 * HomelandEndermen.getByGroupRole(this, OvertakeGroupRole.TELEPORTER).size();
        else return true;
    }
    ...
    private boolean shouldActHostile(Entity entity) {
        ...
        else if (homelandRole == HomelandRole.ISLAND_LEADERS || (groupId != -1 && rand.nextInt(5) != 0)
                || HomelandEndermen.isOvertakeHappening(this))
            return false;
    ...
    public boolean teleportRandomly() {
        return teleportTo(
                posX + (rand.nextDouble() - 0.5D) * 64D,
                posY + (rand.nextInt(64) - 32),
                posZ + (rand.nextDouble() - 0.5D) * 64D);
    }

    public boolean teleportRandomly(double maxDist) {
        return teleportTo(
                posX + (rand.nextDouble() - 0.5D) * 2D * maxDist,
                posY + (rand.nextInt((int) maxDist) - maxDist * 0.5D),
                posZ + (rand.nextDouble() - 0.5D) * 2D * maxDist);
    }

    public boolean teleportToEntity(Entity entity) {
        Vec3 vec = Vec3.createVectorHelper(
                posX - entity.posX,
                boundingBox.minY + (height / 2F) - entity.posY + entity.getEyeHeight(),
    ...
    private boolean teleportTo(double x, double y, double z, boolean ignoreChecks) {
        ...
        if (worldObj.blockExists(tmpPos.x, tmpPos.y, tmpPos.z) || ignoreChecks) {
            ...
            while (!foundTopBlock && tmpPos.y > 0) {
                if (tmpPos.moveDown().getMaterial(worldObj).blocksMovement()
                        && tmpPos.getBlock(worldObj) != BlockList.persegrit)
                    foundTopBlock = true;
                else--posY;
            }
            ...
            if (foundTopBlock) {
                ...
                if ((worldObj.getCollidingBoundingBoxes(this, boundingBox).isEmpty()
                        && !worldObj.isAnyLiquid(boundingBox)) || ignoreChecks) {
                    hasTeleported = true;
                }
            }
        }
        ...
        if (!hasTeleported) {
            ...
            if (!worldObj.isRemote && !ignoreChecks) PacketPipeline.sendToAllAround(
                    this,
                    256D,
                    new C22EffectLine(FXType.Line.ENDERMAN_TELEPORT, oldX, oldY, oldZ, posX, posY, posZ));
            return true;
        }
    }
    ...
    private boolean isPlayerStaringIntoEyes(EntityPlayer player) {
        ...
        if (is != null && is.getItem() == Item.getItemFromBlock(Blocks.pumpkin)) return false;
            ...
            Vec3 eyeVecDiff = Vec3.createVectorHelper(
                    posX - player.posX,
                    boundingBox.minY + (height * 0.5F) - (player.posY + player.getEyeHeight()),
                    posZ - player.posZ);
            double eyeVecLen = eyeVecDiff.lengthVector();
            return playerLook.dotProduct(eyeVecDiff.normalize()) > 1D - 0.025D / eyeVecLen
