        ...
        if (++field_145812_b == 1 && !worldObj.isRemote) {
            ...
            for (int origX = tmpPos.x, origZ = tmpPos.z, x = origX + 2; x < origX + 5; x++) {
                for (int z = origZ - 1; z <= origZ + 1; z++) {
                    if (tmpPos.set(x, tmpPos.y, z).getBlock(worldObj) == BlockList.temple_end_portal)
                        tmpPos.setMetadata(worldObj, 1, 2);
                }
            }
        }
            ...
            if (!worldObj.isRemote) {
                if (deltaY > 4D && rand.nextInt(6 - (deltaY > 6D ? 2 : 0)) == 0) {
                    ...
                    worldObj.createExplosion(
                            this,
                            xx,
                            ItemTempleCaller.templeY + rand.nextFloat() * 8D,
                            zz,
                            1.5F + (float) ((deltaY - 4F) * 0.5F),
                            true);
                }
