    ...
    private void renderAltar() {
        ...
        if (stage == TileEntityEssenceAltar.STAGE_WORKING) {
            ...
            if (is != null) {
                ...
                if (is.getItemSpriteNumber() == 0
                        && BlockRenderHelper.shouldRenderBlockIn3D(Block.getBlockFromItem(is.getItem()))) {
                    GL11.glScalef(0.7F, 0.7F, 0.7F);
                    GL11.glRotatef(requiredItemRot, 0F, 1F, 0F);
                    BlockRenderHelper
                            .renderBlockAsItem(blockRenderer, Block.getBlockFromItem(is.getItem()), is.getItemDamage());
                } else {
        ...
        for (int a = 0; a < 8; a++) {
            ...
            if (index == -1) renderItem(
                    ...
                    ItemSpecialEffects.glyphIndex + a,
                    glyphCols[0],
                    glyphCols[1],
                    glyphCols[2]);
            else if (index > a)
                renderItem(ItemList.special_effects, ItemSpecialEffects.glyphIndex + a, 0.9725F, 0.8265F, 0.225F);
