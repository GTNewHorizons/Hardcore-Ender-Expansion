...
public class EnergyChunkData {
    ...
    public void onUpdate(World world, Random rand) {
        if ((regenTimer == 0 || --regenTimer == 0) && energyLevel < maxEnergyLevel) {
            if ((energyLevel += Math.min(0.025F, Math.sqrt(maxEnergyLevel) * 0.007F)) > maxEnergyLevel)
                energyLevel = maxEnergyLevel;
            regenTimer = (byte) (9 + rand.nextInt(12 + rand.nextInt(6))
                    + MathUtil.floor((16F * energyLevel / maxEnergyLevel) + maxEnergyLevel * 0.2F));
        }

        if ((energyLevel > maxEnergyLevel || (energyLevel > maxEnergyLevel * 0.8F && rand.nextInt(10) == 0))
    ...
    public void onAdjacentInteract(Random rand, EnergyChunkData data) {
        if (rand.nextInt(42) == 0 && data.energyLevel < energyLevel
                && data.energyLevel < data.maxEnergyLevel
                && Math.abs(data.energyLevel - energyLevel) > 0.4F + rand.nextFloat() * 2.5F) {
            float amt = Math.min(
                    energyLevel,
                    Math.min(
                            data.maxEnergyLevel - data.energyLevel,
                            Math.max(
                                    0.01F,
                                    Math.min(
                                            0.2F,
                                            Math.abs(data.energyLevel - energyLevel) * 0.02F
    ...
    public static EnergyChunkData readFromNBT(NBTTagCompound nbt) {
        ...
        if (MathUtil.floatEquals(data.maxEnergyLevel = nbt.getFloat("max"), 0F))
            data.maxEnergyLevel = minSignificantEnergy;
        return data;
    }
}
