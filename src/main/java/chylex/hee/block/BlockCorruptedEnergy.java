...
public class BlockCorruptedEnergy extends Block {
    ...
    private static final byte[] offsetX = new byte[] { -1, 1, 0, 0, 0, 0 }, offsetY = new byte[] { 0, 0, -1, 1, 0, 0 },
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (world.isRemote) {
            ...
            if (world.rand.nextInt(5) == 0) world.spawnParticle(
                    "explode",
                    x + 0.5D,
                    y + 0.5D,
                    z + 0.5D,
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity instanceof EntityLivingBase && !(entity instanceof IBossDisplayData)) {
            ...
            if (world.rand.nextInt(meta >= 10 ? 3 : (meta >= 5 ? 4 : 5)) == 0
                    && (living.hurtTime <= 3 || world.rand.nextInt(7) == 0)
                    && living.getHealth() > 0F) {
                ...
                living.func_110142_aN().func_94547_a(DamageSource.magic, living.prevHealth, 1F); // OBFUSCATED combat
                                                                                                 // tracker, second
                                                                                                 // method
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (world.rand.nextInt(30) == 0) world.spawnParticle(
                "explode",
                x + 0.5D,
                y + 0.5D,
                z + 0.5D,
