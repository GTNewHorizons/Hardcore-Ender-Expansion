    ...
    public static boolean teleportNearby(World world, int x, int y, int z) {
        if (BlockPosM.tmp(x, y, z).getBlock(world) == Blocks.dragon_egg && !world.isRemote) {
            ...
            for (int attempt = 0; attempt < 1000; ++attempt) {
                tmpPos.set(
                        x + world.rand.nextInt(31) - 15,
                        y + world.rand.nextInt(15) - 7,
                        z + world.rand.nextInt(31) - 15);
                ...
                if (tmpPos.getMaterial(world) == Material.air) {
                    ...
                    PacketPipeline.sendToAllAround(
                            ...
                            64D,
                            new C22EffectLine(
                                    FXType.Line.DRAGON_EGG_TELEPORT,
                                    x + 0.5D,
                                    y + 0.5D,
                                    z + 0.5D,
                                    tmpPos.x + 0.5D,
    ...
    public static boolean teleportEntityToPortal(Entity eggEntity) {
        ...
        if (file.isDragonDead()) {
            ...
            if (endWorld == null)
                ...
                PacketPipeline.sendToAllAround(
                        endWorld.provider.dimensionId,
                        coords.x + 0.5D,
                        coords.y + 0.5D,
                        coords.z + 0.5D,
                        64D,
                        new C20Effect(
                                FXType.Basic.DRAGON_EGG_RESET,
                                coords.x + 0.5D,
                                coords.y + 0.5D,
                                coords.z + 0.5D));
                return true;
